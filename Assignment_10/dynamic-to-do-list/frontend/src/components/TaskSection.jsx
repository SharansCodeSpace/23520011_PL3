...
const TaskSection = ({ title, tasks = [], updateTaskStatus, deleteTask, editTask }) => {
    ...
    return (
        <div ref={setNodeRef} className="bg-white p-6 rounded-xl shadow-md w-1/3 flex flex-col items-center hover:shadow-lg transition-transform transform hover:scale-105">
            ...
            {tasks.map((task, index) => (
                <TaskCard key={task.id} task={task} updateTaskStatus={updateTaskStatus} deleteTask={deleteTask} editTask={editTask} />
            ))}
        </div>
    );
};
